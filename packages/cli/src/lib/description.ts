...
export class TaskDescription {
    ...
    static create(taskData: CreateTaskData): TaskDescription {
        ...
        const taskDir = join(process.cwd(), '.rover', 'tasks', taskData.id.toString());
    ...
    static load(taskId: number): TaskDescription {
        ...
        try {
            ...
            if (error instanceof SyntaxError) {
                throw new TaskSchemaError(`Invalid JSON in task ${taskId}: ${error.message}`);
            }
    ...
    private static getTaskDescriptionPath(taskId: number): string {
        return join(process.cwd(), '.rover', 'tasks', taskId.toString(), 'description.json');
    }
    ...
    private static migrate(data: any, taskId: number): TaskDescriptionSchema {
        ...
        migrated.id = typeof data.id === 'string' ? parseInt(data.id, 10) : (data.id || taskId);
    ...
    get status(): TaskStatus { return this.data.status; }
    ...
    get failedAt(): string | undefined { return this.data.failedAt; }
    get lastIterationAt(): string | undefined { return this.data.lastIterationAt; }
    get lastStatusCheck(): string | undefined { return this.data.lastStatusCheck; }
    get iterations(): number { return this.data.iterations; }
    get worktreePath(): string { return this.data.worktreePath; }
    get branchName(): string { return this.data.branchName; }
    get containerId(): string | undefined { return this.data.containerId; }
    get executionStatus(): string | undefined { return this.data.executionStatus; }
    get runningAt(): string | undefined { return this.data.runningAt; }
    get errorAt(): string | undefined { return this.data.errorAt; }
    get exitCode(): number | undefined { return this.data.exitCode; }
     ...
     */
    updateExecutionStatus(status: string, metadata?: { exitCode?: number, error?: string }): void {
    ...
    private validate(): void {
        ...
        if (typeof this.data.iterations !== 'number') errors.push('iterations must be a number');
        ...
        const dateFields = ['createdAt', 'startedAt', 'completedAt', 'failedAt', 'lastIterationAt', 'lastStatusCheck'];
